...
const boatStatusLabel=s=>({not_booked:'Not booked',discussed:'Discussed',quoted:'Quoted',booked:'Booked',confirmed:'Confirmed',booked_elsewhere:'Booked elsewhere',enquiry:'Enquiry',provisional:'Provisional',completed:'Completed',cancelled:'Cancelled'}[s]||'Discussed');
...
const supplierPaymentDateFor=b=>{
  if(!b)return null;
  if((b.booking_type||'villa_stay')==='villa_stay')return b.supplier_payment_due_date||b.arrival_date||null;
  return b.supplier_payment_due_date||bookingPrimaryDate(b)||null;
};
...
function supplierPanelHtml(type){
  const rows=resources.filter(r=>r.resource_type===type&&r.active!==false&&!String(r.name||'').startsWith('__')).sort(resourceNameSort);
  ...
  return `<div class="panel resource-panel supplier-panel">
    ...
    <div class="resource-list">${rows.map(r=>`<div class="resource-row"><span>${esc(r.name)}</span><button class="text-button" data-resource-toggle="${r.id}" data-resource-active="true">Remove</button></div>`).join('')||'<div class="empty">No suppliers yet.</div>'}</div>
...
function applyBookingTypeTemplate(){
  ...
  document.querySelectorAll('.villa-shopping-module').forEach(el=>el.classList.toggle('hidden',type==='villa_stay'));
...
function buildPriorities(){
  ...
  activeBookings().forEach(b=>{
    ...
    const supplier=supplierOwedFor(b),supplierDate=supplierPaymentDateFor(b),supplierDays=daysFromToday(supplierDate);
...
function supplierOperationalTime(b){
  if(!b)return '';
  const due=supplierPaymentDateFor(b);
  if(!due)return '';
  ...
  if(type==='boat_charter'){
    const bt=bookingBoat(b.id);
    eventDate=bt?.charter_date||b.service_date||'';
    eventTime=bt?.start_time||b.start_time||'';
  }else if(type==='private_chef'){
    const ch=bookingChef(b.id);
    eventDate=ch?.event_date||b.service_date||'';
    eventTime=ch?.event_time||ch?.meal_time||ch?.service_time||ch?.start_time||'';
  }else if(type==='villa_stay'){
    const ch=bookingChef(b.id);
    const bt=bookingBoat(b.id);
    if(ch && String(due).slice(0,10)===String(ch.event_date||'').slice(0,10)){
      eventDate=ch.event_date||'';
      eventTime=ch.event_time||ch.meal_time||ch.service_time||ch.start_time||'';
    }else if(bt && String(due).slice(0,10)===String(bt.charter_date||'').slice(0,10)){
      eventDate=bt.charter_date||'';
      eventTime=bt.start_time||'';
    }else{
      eventDate=b.arrival_date||'';
      eventTime=b.arrival_time||'';
    }
  }else{
    eventDate=b.service_date||b.event_date||'';
    eventTime=b.event_time||b.service_time||b.start_time||'';
  }

  if(String(due).slice(0,10)!==String(eventDate).slice(0,10))return '';
  return eventTime||'';
}
...
function renderDailyOperations(){
  ...
  moneyDue.supplier.forEach(b=>{const d=supplierPaymentDateFor(b),dueTime=supplierOperationalTime(b);push(d,dueTime||'',dailyCard('SUPPLIER','Supplier payment due',dueTime||'All day','',`${money(supplierOwedFor(b),supplierCurrencyFor(b))} • ${esc(primaryResource(b))}`,'',b.id,'supplier'),b)});
...
function buildOperationalEvents(){
  ...
  activeBookings().forEach(b=>{
    ...
    if(supplierDue>0){const d=b.supplier_payment_due_date||nextDue||primaryDate||b.arrival_date||b.service_date,dueTime=supplierOperationalTime(b);addOperationalEvent(events,{booking:b,date:d,time:dueTime,title:'Supplier payment due',detail:`${money(supplierDue,supplierCurrencyFor(b))} • ${resource}`,category:'financial',status:daysFromToday(d)<0?'urgent':'soon'});}
    if(type==='villa_stay'){
...
function renderOperationsCentre(){
  ...
  feed.innerHTML=[...months.entries()].map(([month,days])=>{
    ...
    const dayHtml=[...days.entries()].map(([day,items])=>`<section class="operations-day">
      <div class="operations-day-head"><strong>${longConsoleDate(day)}</strong></div>
...
function switchView(name){
  ...
  document.querySelectorAll('.nav-item,.sidebar .active').forEach(button=>button.classList.remove('active'));
...
function applyMarbellaHideawayPaymentDefaults(force=false){
  ...
  // Villa supplier rule: the full rental is payable to the villa supplier
  // on the guest's arrival date, in the same currency as the booking.
  if($('supplierAmountOwed'))$('supplierAmountOwed').value=total.toFixed(2);
  if($('supplierPaymentDueDate'))$('supplierPaymentDueDate').value=arrival;
  if($('supplierCurrency'))$('supplierCurrency').value=$('bookingCurrency')?.value||'GBP';
  if($('supplierPaymentDueDateHelp'))$('supplierPaymentDueDateHelp').textContent='Villa supplier payment is due on the arrival date.';
  ...
  // Only seed the suggested deposit when the field is empty/zero (or when the
  // user explicitly changes the payment strategy). Never overwrite a deposit
  // amount the user is actively typing.
  if(force||currentDeposit===0)$('depositPaid').value=targetDeposit.toFixed(2);

  const deposit=Number($('depositPaid')?.value||0);
  const finalDate=isoDateMinusDays(arrival,meta.finalDays);
  const partialInitialDeposit=!staged&&targetDeposit>0&&deposit>0&&deposit<targetDeposit;

  // A standard 50% plan can still begin with a smaller first payment.
  // In that case the remainder of the 50% deposit becomes a second-deposit stage.
  if(partialInitialDeposit){
    const secondAmount=Math.max(0,targetDeposit-deposit);
    const finalAmount=Math.max(0,total-targetDeposit);
    ...
    $('balanceDueDateWrap')?.classList.remove('is-hidden');
    ...
    if($('finalPaymentAmount'))$('finalPaymentAmount').value=money(finalAmount,$('bookingCurrency')?.value||'GBP');
    if($('stagedPaymentActionLabel'))$('stagedPaymentActionLabel').textContent='Second deposit paid date';
    if($('stagedPaymentActionHelp'))$('stagedPaymentActionHelp').textContent='Enter the date the second deposit is received.';
    if($('stagedPaymentPaidDate')&&!$('stagedPaymentPaidDate').value)$('stagedPaymentPaidDate').value=booking?.second_deposit_paid_date||'';
    if($('recordStagedPayment')){
      $('recordStagedPayment').textContent='Record second deposit as paid';
      $('recordStagedPayment').classList.toggle('is-hidden',!existing);
  ...
  if(staged){
    ...
    const savedStage=existing?((booking?.second_deposit_paid_date&&meta?.stageName==='second_deposit')?'final_balance':(booking?.next_payment_stage||$('nextPaymentStage')?.value)):'further_deposit';
    ...
    $('stagedPaymentActionWrap')?.classList.toggle('is-hidden',!existing);
    ...
    if($('recordStagedPayment')){
      $('recordStagedPayment').textContent=meta.stageName==='second_deposit'?'Record second deposit as paid':'Record further payment as paid';
      $('recordStagedPayment').classList.toggle('is-hidden',!canRecord);
    }
  }else{
...
function updateVillaConciergeVisibility(){
  if($('bookingType')?.value!=='villa_stay')return;
  const hideByStatus=(id,statusId)=>{
    const el=$(id),status=$(statusId)?.value;
    if(el)el.classList.toggle('is-hidden',!status||status==='not_booked');
  };
  hideByStatus('boatEditorModule','boatStatus');
  hideByStatus('chefEditorModule','chefStatus');
  hideByStatus('decorEditorModule','decorStatus');
  hideByStatus('shopEditorModule','shopStatus');
  hideByStatus('beachEditorModule','beachStatus');
  hideByStatus('entEditorModule','entStatus');
  for(let i=1;i<=3;i++){
    const slot=document.querySelector(`[data-restaurant-slot="${i}"]`);
    const status=$(`restaurant${i}Status`)?.value;
    if(slot)slot.classList.toggle('is-hidden',!status||status==='not_booked');
  }
  const restaurantHeading=document.querySelector('.restaurant-editor-list')?.previousElementSibling;
  const anyRestaurant=[1,2,3].some(i=>($(`restaurant${i}Status`)?.value||'not_booked')!=='not_booked');
  if(restaurantHeading)restaurantHeading.classList.toggle('is-hidden',!anyRestaurant);
}
...
function hydrateSavedBoatEditFields(b){
  ...
  if($('boatStatus')){
    const savedBoatStatus=effectiveBoatStatus(b);
    $('boatStatus').value=(b.booking_type==='villa_stay'&&['not_booked','enquiry','provisional','cancelled'].includes(savedBoatStatus))?'discussed':savedBoatStatus;
    if(!$('boatStatus').value)$('boatStatus').value=b.booking_type==='villa_stay'?'discussed':'confirmed';
  }
}
...
updateBoatFinancials();updateChefFinancials();['decor','shop','beach','ent'].forEach(updateExperienceBalance);updateVillaConciergeVisibility();
...
document.addEventListener('click',async e=>{
  const btn=e.target.closest('[data-resource-toggle]');
  if(!btn)return;
  e.preventDefault();
  e.stopPropagation();
  const id=btn.dataset.resourceToggle;
  ...
  const {data,error}=await supabaseClient
    ...
    .maybeSingle();
  if(error){
    $('resourceMessage').textContent=error.message;
    btn.disabled=false;
    return;
  }
  if(!data){
    $('resourceMessage').textContent='Supplier could not be removed. No database row was updated.';
    btn.disabled=false;
    return;
  }
  item.active=false;
  $('resourceMessage').textContent=`${item.name} removed.`;
  renderResources();
  await loadResources();
  renderResources();
});
