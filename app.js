...
function paymentSummaryFor(b){
  const total=Number(b?.total_rental||0),paid=paidForCurrency(b,bookingCurrency(b)),balance=Math.max(0,total-paid);
  const notes=String(b?.payment_strategy_notes||'').trim();
  if(b?.payment_strategy==='fully_paid'||balance<=0)return notes||'Fully paid.';
  const nextAmount=nextPaymentAmountFor(b),nextDate=nextPaymentDateFor(b);
  let summary='';
  if(b?.payment_strategy==='pay_later')summary='Nothing to pay until the agreed later date.';
  else if(b?.payment_strategy==='staged')summary='Staged payment arrangement.';
  else if(b?.payment_strategy==='standard_50_30')summary='50% deposit, with the remaining balance due 30 days before arrival.';
  else summary='Custom payment arrangement.';
  if(nextAmount>0)summary+=` Next payment ${money(nextAmount,bookingCurrency(b))}${nextDate?` due ${date(nextDate)}`:''}.`;
  else if(balance>0)summary+=` ${money(balance,bookingCurrency(b))} outstanding.`;
...
function updateBoatCurrencyCalculations(){
  ...
  if($('nextPaymentAmount'))$('nextPaymentAmount').value=balanceEur.toFixed(2);
}
function applyBoatWorkflowDefaults(b=null){
  ...
  if(!isBoat){
    ...
    if($('openingPaymentWrap'))$('openingPaymentWrap').classList.remove('hidden');
  ...
  if($('openingPaymentWrap'))$('openingPaymentWrap').classList.add('hidden');
...
async function init(){
  try{
    ...
    supabaseClient.auth.onAuthStateChange(async(_e,s)=>s?await enterApp(s.user):show('loginView'));
  }catch(e){console.error(e);$('boot').textContent='Unable to open the app. Please refresh.';}
}
function show(id){['boot','setupView','loginView','appView'].forEach(x=>$(x)?.classList.add('hidden'));$(id).classList.remove('hidden');}
...
function applyBookingTypeTemplate(){
  ...
  $('modalTitle').textContent=editing?cfg.edit:cfg.page;
...
function dailyMoneyDueForDate(day){
  ...
  activeBookings().forEach(b=>{
    ...
    const supplierDate=nextPaymentDateFor(b)||bookingPrimaryDate(b);
...
function renderDailyOperations(){
  ...
  moneyDue.guest.forEach(b=>push(nextPaymentDateFor(b),'08:00',dailyCard('GUEST PAYMENT','Guest payment due','08:00','',`${money(nextPaymentAmountFor(b),bookingCurrency(b))} due`,paymentSummaryFor(b),b.id,'money'),b));
  moneyDue.supplier.forEach(b=>{const d=nextPaymentDateFor(b)||bookingPrimaryDate(b);push(d,'08:30',dailyCard('SUPPLIER','Supplier payment due','08:30','',`${money(supplierOwedFor(b),supplierCurrencyFor(b))} • ${esc(primaryResource(b))}`,'',b.id,'supplier'),b)});
  ...
  return `<div class="grouped-bookings">${groups.map(group=>{
    ...
    return `<article class="guest-itinerary-card">
      <button class="guest-itinerary-head" type="button" onclick="this.closest('.guest-itinerary-card').classList.toggle('expanded')">
        ...
        <span><small>Next payment</small><strong>${next?money(nextPaymentAmountFor(next),bookingCurrency(next)):'—'}</strong>${nextPaymentDateFor(next)?`<small>${date(nextPaymentDateFor(next))}</small>`:''}</span>
...
function applyMarbellaHideawayPaymentDefaults(force=false){
  const isHideaway=$('bookingType')?.value==='villa_stay'&&String($('villaName')?.value||'').trim().toLowerCase()==='marbella hideaway';
  if(!isHideaway)return;
  const total=Number($('totalRental')?.value||0);
  const paid=Number($('depositPaid')?.value||0);
  const remaining=Math.max(0,total-paid);
  if(force||!Number($('nextPaymentAmount')?.value||0))$('nextPaymentAmount').value=remaining.toFixed(2);
  if(force||!$('nextPaymentDueDate')?.value)$('nextPaymentDueDate').value=isoDateMinusDays($('arrivalDate')?.value,30);
  if(force||!$('nextPaymentStage')?.value)$('nextPaymentStage').value='final_balance';
}
...
function repopulateSavedFinancialFields(b){
  ...
  if($('balanceDueDate'))$('balanceDueDate').value=b.balance_due_date||b.next_payment_due_date||(
    b.booking_type==='boat_charter' ? (bookingBoat(b.id)?.charter_date||b.service_date||'') : ''
  );
  if($('nextPaymentDueDate')&&!$('nextPaymentDueDate').value)$('nextPaymentDueDate').value=b.next_payment_due_date||b.balance_due_date||'';
...
function openModal(b=null,options={}){$('supplierCurrency')?.removeAttribute('data-overridden');
  $('bookingForm').reset();$('bookingId').value=b?.id||'';$('customerId').value=b?.customer_id||options.customer?.customer_id||options.customer?.key||'';$('itineraryId').value=b?.itinerary_id||options.itineraryId||options.customer?.itinerary_id||'';renderLinkedCustomerPanel(options.customer||null);$('modalTitle').textContent=b?'Edit booking':'Add booking';
...
function customerItineraryRows(customer){
  return [...customer.bookings].sort((a,b)=>new Date(bookingPrimaryDate(a)||'9999-12-31')-new Date(bookingPrimaryDate(b)||'9999-12-31')).map(x=>{
    ...
    return `<article class="customer-itinerary-row">
      ...
      <div><span>Next</span><strong>${next?money(next,bookingCurrency(x)):'—'}</strong><small>${nextPaymentDateFor(x)?date(nextPaymentDateFor(x)):''}</small></div>
      <button class="button secondary compact" onclick="openDetail('${x.id}')">Open</button>
    </article>`;
  }).join('');
}
...
function aiConciergeInsights(b){
  ...
  bs.forEach(x=>{
    ...
    const supplier=supplierOwedFor(x),sd=nextPaymentDateFor(x)||bookingPrimaryDate(x),sDays=daysFromToday(sd);
...
$('bookingForm').addEventListener('submit',async e=>{
  ...
  const openingPayment=type==='boat_charter'?Number($('depositPaid').value||0):Number($('depositReceived').value||0);
  refreshGeneratedTitle();
  ...
  let result=id?await supabaseClient.from('bookings').update(payload).eq('id',id).select().single():await supabaseClient.from('bookings').insert({...payload,deposit_received:0}).select().single();
...
['totalRental','depositPaid','arrivalDate'].forEach(id=>$(id)?.addEventListener('change',()=>applyMarbellaHideawayPaymentDefaults(false)));
$('depositPaid')?.addEventListener('change',()=>{if(Number($('depositPaid').value||0)>0&&!$('depositPaidDate').value)$('depositPaidDate').value=new Date().toISOString().slice(0,10);});
...
['totalRental','depositPaid','nextPaymentAmount','nextPaymentDueDate','paymentStrategy','paymentStrategyNotes','bookingCurrency'].forEach(id=>{
...
$('serviceDate')?.addEventListener('change',()=>{if(($('bookingType')?.value||'')==='boat_charter'){syncBoatBookingFields(true);if($('nextPaymentDueDate'))$('nextPaymentDueDate').value=$('serviceDate').value;}});
